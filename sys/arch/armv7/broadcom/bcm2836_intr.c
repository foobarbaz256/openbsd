...
void	*bcm_intc_intr_establish(int, int, int (*)(void *),
    void *, char *);
void	*bcm_intc_intr_establish_fdt_idx(int, int, int, int (*)(void *),
    void *, char *);
...
bcm_intc_match(struct device *parent, void *cfdata, void *aux)
{
	...
	if (!strcmp(buffer, "brcm,bcm2708-armctrl-ic") ||
	    !strcmp(buffer, "brcm,bcm2835-armctrl-ic") ||
	    !strcmp(buffer, "broadcom,bcm2835-armctrl-ic"))
		return 1;
...
bcm_intc_attach(struct device *parent, struct device *self, void *args)
{
	...
	arm_set_intr_handler_fdt(fa->fa_node, bcm_intc_intr_establish_fdt_idx);
...
void *
bcm_intc_intr_establish_fdt_idx(int node, int idx, int level,
    int (*func)(void *), void *arg, char *name)
{
	...
	irq = betoh32(ints[idx * 2 + 1]);
	if (betoh32(ints[idx * 2]) == 0)
		irq += BANK0_START;
	else if (betoh32(ints[idx * 2]) == 1)
		irq += BANK1_START;
	else if (betoh32(ints[idx * 2]) == 2)
		irq += BANK2_START;
	else if (betoh32(ints[idx * 2]) == 3)
		irq += LOCAL_START;
	else
